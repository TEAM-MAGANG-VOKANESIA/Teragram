    ...
    <div class="h-screen bg-gray-100 flex flex-col justify-center items-center">
        <div class="bg-white mb-3 border border-gray-300 w-80 pt-8 pb-4 flex flex-col items-center">
            ...
            <form class="w-64 flex flex-col gap-1 mt-8" method="post" action="{{ route("$routeName") }}">
                ...
                {{-- LOGIN TERAGRAM --}}
                <button
                    class="mt-2 text-sm text-center bg-blue-600 hover:bg-blue-400 text-white py-1 rounded-lg font-medium">{{$button}}</button>
        ...
        <div class="bg-white border border-gray-300 w-80 p-4 flex items-center justify-center">
            <h1 class="text-center text-xs font-medium">
                {{$linkTitle}}<a href="{{$link}}" class="text-xs font-medium text-blue-700 ml-1 mb-2">{{$linkAction}}</a>
            </h1>
        </div>
